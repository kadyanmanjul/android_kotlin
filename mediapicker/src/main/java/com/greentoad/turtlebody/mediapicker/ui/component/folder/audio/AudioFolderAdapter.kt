...
class AudioFolderAdapter: RecyclerView.Adapter<AudioFolderAdapter.FolderVewHolder>() {
    ...
    inner class FolderVewHolder(itemView: View): RecyclerView.ViewHolder(itemView){
        fun bind(pData: AudioFolder){
            ...
            itemView.findViewById<TextView>(R.id.item_folder_txt_folder_name).text = pData.name
            itemView.findViewById<TextView>(R.id.item_folder_txt_total_items).text = "${pData.contentCount} items"

            itemView.findViewById<FrameLayout>(R.id.item_folder_fl).visibility = View.GONE

            Glide.with(itemView)
                .load(R.drawable.tb_media_picker_ic_music_folder)
                .into(itemView.findViewById(R.id.item_folder_icon))
