...
import com.joshtalks.joshskills.premium.ui.launch.getStringOrNull
...
import com.joshtalks.joshskills.premium.ui.special_practice.utils.COUPON_CODE
import com.joshtalks.joshskills.premium.ui.special_practice.utils.FLOW_FROM
...
class DeepLinkRedirectUtil(private val jsonParams: JSONObject) {
    ...
    fun initCourseOnboarding(isSpecialCourse: Boolean) {
        if (isSpecialCourse && jsonParams.has(DeepLinkData.COURSE_ID.key) && jsonParams.has(DeepLinkData.PLAN_ID.key))
            com.joshtalks.joshskills.premium.core.PrefManager.put(
                key = com.joshtalks.joshskills.premium.core.SPECIFIC_ONBOARDING,
            ...
            com.joshtalks.joshskills.premium.core.PrefManager.put(
                com.joshtalks.joshskills.premium.core.FT_COURSE_ONBOARDING, jsonParams.getString(DeepLinkData.TEST_ID.key))
    }
    ...
    fun getCourseDetailsActivityIntent(activity: Activity) = CourseDetailsActivity.getIntent(
        ...
        if (User.getInstance().isVerified.not()) {
            if (com.joshtalks.joshskills.premium.core.PrefManager.getBoolValue(
                    com.joshtalks.joshskills.premium.core.IS_GUEST_ENROLLED, false) &&
                com.joshtalks.joshskills.premium.core.PrefManager.getBoolValue(
                    com.joshtalks.joshskills.premium.core.IS_PAYMENT_DONE, false).not()
            )
                activityList.add(Intent(getInboxActivityIntent(activity)))
            else if (com.joshtalks.joshskills.premium.core.PrefManager.getBoolValue(
                    com.joshtalks.joshskills.premium.core.IS_PAYMENT_DONE, false))
                activityList.add(Intent(activity, SignUpActivity::class.java))
            else if (com.joshtalks.joshskills.premium.core.PrefManager.getBoolValue(
                    com.joshtalks.joshskills.premium.core.IS_FREE_TRIAL, false, false))
                activityList.add(
                    Intent(
                        activity,
    ...
    private suspend fun getP2PActivityIntent(activity: Activity) =
        if (com.joshtalks.joshskills.premium.core.PrefManager.getBoolValue(
                com.joshtalks.joshskills.premium.core.IS_FREE_TRIAL_ENDED
            ))
            getConversationActivityIntent(activity)
    ...
    suspend fun handleDeepLink(activity: CoreJoshActivity, userProfileNotComplete: Boolean): RedirectAction? =
        when {
            ...
            redirectFromDeepLink(activity, com.joshtalks.joshskills.premium.core.PrefManager.getBoolValue(
                com.joshtalks.joshskills.premium.core.IS_FREE_TRIAL
            )) -> null
    ...
    private inline fun isSpecificOnboarding(): Boolean =
        com.joshtalks.joshskills.premium.core.PrefManager.hasKey(
    ...
    private fun isFreeTrialOnboarding(): Boolean =
        com.joshtalks.joshskills.premium.core.PrefManager.hasKey(
            com.joshtalks.joshskills.premium.core.FT_COURSE_ONBOARDING, isConsistent = true) ||
