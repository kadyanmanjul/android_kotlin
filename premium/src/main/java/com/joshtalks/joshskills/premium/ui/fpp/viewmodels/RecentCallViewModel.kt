...
import com.joshtalks.joshskills.premium.ui.fpp.repository.RecentCallsRepository
            ...
            try {
                ...
                if (response.isSuccessful && response.body()?.arrayList != null) {
                    withContext(mainDispatcher) {
                        if (isFirstTime) {
                            adapter.addRecentCallToList(response.body()?.arrayList!!, PrefManager.getBoolValue(
                                IS_FREE_TRIAL
                            ))
                            isFirstTime = false
                        } else {
