...
import com.joshtalks.joshskills.premium.core.FirebaseRemoteConfigKey.Companion.IS_MISSED_NOTIFICATION_ACTIVE
...
import com.joshtalks.joshskills.premium.core.notification.FCM_ACTIVE
...
import com.joshtalks.joshskills.premium.ui.launch.LauncherScreenImpressionService
...
class NotificationEngagementSyncWorker(val context: Context, workerParams: WorkerParameters) :
    ...
    override suspend fun doWork(): Result {
        try {
            ...
            if (shouldFetchClientData()) {
                ...
                PrefManager.put(
                    DAILY_NOTIFICATION_COUNT, PrefManager.getIntValue(
                        DAILY_NOTIFICATION_COUNT, defValue = 0) + 1)
            }
        } catch (ex: Exception) {
            ex.printStackTrace()
        }
    ...
    private fun shouldSendEnabledAnalytics(): Boolean {
        val count = PrefManager.getIntValue(NOTIFICATION_STATUS_COUNT)
        val timeDifference = System.currentTimeMillis() - PrefManager.getLongValue(
            NOTIFICATION_LAST_TIME_STATUS
        )
