...
import com.joshtalks.joshskills.premium.core.LOCAL_NOTIFICATION_INDEX
...
class LocalNotificationDismissEventReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context?, mIntent: Intent?) {
        Timber.d("Local Notification Dismissed ${mIntent?.extras?.get("extras")}  LOCAL_NOTIFICATION_INDEX: ${
            PrefManager.getIntValue(
                LOCAL_NOTIFICATION_INDEX, defValue = 0)}")
        try {
            PrefManager.put(
