...
import com.joshtalks.joshskills.common.repository.local.eventbus.VideoDownloadedBus
        ...
        executor.execute {
            try {
                ...
                if (downloadStatus == DOWNLOAD_STATUS.FAILED || downloadStatus == DOWNLOAD_STATUS.DOWNLOADED) {
                    com.joshtalks.joshskills.common.messaging.RxBus2.publish(VideoDownloadedBus(chatModel))
                }
            } catch (ex: Exception) {
                ex.printStackTrace()
            }
            try {
                ...
                if (downloadStatus == DOWNLOAD_STATUS.FAILED || downloadStatus == DOWNLOAD_STATUS.DOWNLOADED) {
                    com.joshtalks.joshskills.common.messaging.RxBus2.publish(
                        VideoDownloadedBusForLessonQuestion(lessonQuestion)
                    )
                }
            } catch (ex: Exception) {
                ex.printStackTrace()
