package com.joshtalks.joshskills.common.ui.assessment
...
class AssessmentActivity : CoreJoshActivity() {
            ...
            if ((status == AssessmentStatus.COMPLETED) && (viewModel.getAssessmentType() == null || viewModel.getAssessmentType() == AssessmentType.TEST)) {
                viewModel.assessmentLiveData.value?.let {
                    if (it.questionList.filter { it.question.choiceType == ChoiceType.MATCH_TEXT }.size > 0) {
                        com.joshtalks.joshskills.common.messaging.RxBus2.publish(MatchTheFollowingSubmitEvent(it.assessment.remoteId))
                    }
                }
        ...
        viewModel.assessmentLiveData.value?.let { assessmentWithRelations ->
            when (assessmentButtonClick) {
                ...
                AssessmentButtonClick.BACK_TO_SUMMARY -> {
                    com.joshtalks.joshskills.common.messaging.RxBus2.publish(AssessmentLastQuestionSubmitEvent(questionId))
    ...
    private fun onSubmit(assessmentQuestion: AssessmentQuestionWithRelations) {
        ...
        if (assessmentQuestion.question.choiceType == ChoiceType.FILL_IN_THE_BLANKS_TEXT) {
            com.joshtalks.joshskills.common.messaging.RxBus2.publish(FillInTheBlankSubmitEvent(assessmentQuestion.question.remoteId))
        } else if (assessmentQuestion.question.choiceType == ChoiceType.MATCH_TEXT) {
            com.joshtalks.joshskills.common.messaging.RxBus2.publish(MatchTheFollowingSubmitEvent(assessmentQuestion.question.remoteId))
        } else com.joshtalks.joshskills.common.messaging.RxBus2.publish(McqSubmitEvent(assessmentQuestion.question.remoteId))
    ...
    private fun onNext(
        ...
        if (isLastQuestion) {
            com.joshtalks.joshskills.common.messaging.RxBus2.publish(AssessmentLastQuestionSubmitEvent(assessmentQuestionWithRelations.question.remoteId))
    ...
    private fun subscribeRxBus() {
        ...
        compositeDisposable.add(
            com.joshtalks.joshskills.common.messaging.RxBus2.listenWithoutDelay(TestItemClickedEventBus::class.java)
        ...
        compositeDisposable.add(
            com.joshtalks.joshskills.common.messaging.RxBus2.listenWithoutDelay(AssessmentButtonClickEvent::class.java)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe {
                    handleButtonClicks(
