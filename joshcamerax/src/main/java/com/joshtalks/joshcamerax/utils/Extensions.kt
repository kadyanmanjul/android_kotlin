...
fun ViewGroup.circularClose(button: ImageButton, action: () -> Unit = {}) {
    ViewAnimationUtils.createCircularReveal(
        ...
        duration = 500
        doOnStart { action() }
        doOnEnd { this@circularClose.visibility = GONE }
    }.start()
}
...
fun Fragment.send(file: File) {
//    val share = Intent(Intent.ACTION_SEND)
//    share.type = "image/*"
//    share.putExtra(Intent.EXTRA_STREAM, Uri.parse(file.absolutePath))
//    startActivity(Intent.createChooser(share, title))
    //getCompressImage(file.absolutePath)
    val act=this.activity as JoshCameraActivity
    val resultIntent=Intent()
    val paths=arrayListOf(file.absoluteFile.toString())
    resultIntent.putStringArrayListExtra(JoshCameraActivity.IMAGE_RESULTS,paths)
    act.setResult(RESULT_OK,resultIntent)
    act.finish()

}

fun getCompressImage() {
