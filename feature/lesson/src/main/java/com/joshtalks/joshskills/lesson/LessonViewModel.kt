...
import com.joshtalks.joshskills.common.core.custom_ui.m4aRecorder.M4ABaseAudioRecording
...
import com.joshtalks.joshskills.common.repository.server.UpdateLessonResponse
...
class LessonViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val lessonNetworkService: LessonService by lazy {
        AppObjectController.retrofit.create(LessonService::class.java)
    }
    ...
    fun getVideoData() {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val response = lessonNetworkService.getIntroSpeakingVideo()
                if (response.isSuccessful) {
...
//        blockLiveData.postValue(true)
    ...
    private suspend fun getLessonFromDB(lessonId: Int): LessonModel? {
        return withContext(viewModelScope.coroutineContext + Dispatchers.IO) {
            return@withContext AppObjectController.appDatabase.lessonDao().getLesson(lessonId)
        }
    }
    ...
    private suspend fun getQuestionsFromDB(lessonId: Int): List<LessonQuestion> {
        return withContext(viewModelScope.coroutineContext + Dispatchers.IO) {
            try {
                val lessonQuestions =
                    AppObjectController.appDatabase.lessonQuestionDao().getQuestionsForLesson(lessonId)
                lessonQuestions.forEach { lessonQuestion ->

                    val audioList =
                        AppObjectController.appDatabase.chatDao().getAudiosOfQuestion(lessonQuestion.id)
                    ...
                    when (lessonQuestion.materialType) {
                        ...
                        LessonMaterialType.IM ->
                            lessonQuestion.imageList =
                                AppObjectController.appDatabase.chatDao().getImagesOfQuestion(lessonQuestion.id)

                        LessonMaterialType.VI ->
                            lessonQuestion.videoList =
                                AppObjectController.appDatabase.chatDao().getVideosOfQuestion(lessonQuestion.id)
                        ...
                        LessonMaterialType.PD ->
                            lessonQuestion.pdfList =
                                AppObjectController.appDatabase.chatDao().getPdfOfQuestion(lessonQuestion.id)
    ...
    private suspend fun saveQuestionToDB(question: LessonQuestion) {
        return withContext(viewModelScope.coroutineContext + Dispatchers.IO) {
            AppObjectController.appDatabase.lessonQuestionDao().insertQuestionForLesson(question)
            ...
            question.optionsList?.let {
                ...
                AppObjectController.appDatabase.chatDao().insertOptionTypeMessageList(it)
            }
    ...
    fun updateSectionStatus(lessonId: Int, status: LESSON_STATUS, tabPosition: Int) {
        viewModelScope.launch(Dispatchers.IO) {
            when (tabPosition) {
                GRAMMAR_POSITION -> {
                    AppObjectController.appDatabase.lessonDao().updateGrammarSectionStatus(lessonId, status)
                ...
                VOCAB_POSITION -> {
                    AppObjectController.appDatabase.lessonDao().updateVocabularySectionStatus(lessonId, status)
                    lessonLiveData.postValue(
                        lessonLiveData.value?.apply {
                ...
                READING_POSITION -> {
                    AppObjectController.appDatabase.lessonDao().updateReadingSectionStatus(lessonId, status)
                    lessonLiveData.postValue(
                        lessonLiveData.value?.apply {
                ...
                SPEAKING_POSITION -> {
                    ...
                    AppObjectController.appDatabase.lessonDao().updateSpeakingSectionStatus(lessonId, status)
                    lessonLiveData.postValue(
                        lessonLiveData.value?.apply {
                ...
                TRANSLATION_POSITION -> {
                    AppObjectController.appDatabase.lessonDao().updateTranslationSectionStatus(lessonId, status)
                    lessonLiveData.postValue(
                        lessonLiveData.value?.apply {
            ...
            try {
                ...
                if (isVideoPercentComplete) {
                    ...
                    if (resp.isSuccessful && resp.body() != null) {
                        ...
                        AppObjectController.appDatabase.lessonQuestionDao().updateQuestionStatus("$questionId", status)
                        updateLessonStatus()
                        AppObjectController.appDatabase.lessonQuestionDao().getLessonQuestionById(questionId!!)
                        return@launch
                    }
                } else {
    ...
    fun updateQuestionInLocal(question: LessonQuestion) {
        viewModelScope.launch(Dispatchers.IO) {
            AppObjectController.appDatabase.lessonQuestionDao().updateQuestionObject(question)
        }
    }
    ...
    fun getPointsForVocabAndReading(questionId: String) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val response = lessonNetworkService.getSnackBarText(questionId)
    ...
    fun addTaskToService(requestEngage: RequestEngage, pendingTaskType: PendingTask) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            com.joshtalks.joshskills.common.util.FileUploadService.uploadSinglePendingTasks(
    ...
    fun startRecord(): Boolean {
        AppDirectory.tempRecordingFile().let {
            recordFile = it
            com.joshtalks.joshskills.common.util.AudioRecording.audioRecording.startPlayer(recordFile)
            return@let true
        }
        return false
    }
    ...
    fun stopRecording() {
        com.joshtalks.joshskills.common.util.AudioRecording.audioRecording.stopPlaying()
    }
    ...
    fun getPracticeAfterUploaded(id: String, callback: (LessonQuestion?) -> Unit) {
        viewModelScope.launch(Dispatchers.IO) {
            callback.invoke(AppObjectController.appDatabase.lessonQuestionDao().getUpdatedLessonQuestion(id))
        }
    }
        ...
        viewModelScope.launch(Dispatchers.IO) {
            lessonLiveData.postValue(
                lessonLiveData.value?.apply {
                    ...
                    AppObjectController.appDatabase.lessonDao().updateLessonStatus(this.id, lessonStatus)
                }
            )
        }
    }
    ...
    suspend fun updateLesson(lesson: LessonModel) {
        viewModelScope.launch(Dispatchers.IO) {
            AppObjectController.appDatabase.lessonDao().insertSingleItem(lesson)
            lessonLiveData.postValue(lesson)
        }
    }
    ...
    fun isFavoriteCallerExist() {
        viewModelScope.launch(Dispatchers.IO) {
            delay(250)
            val count = AppObjectController.appDatabase.favoriteCallerDao().getCountOfFavoriteCaller()
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                if (response != null) {
                    ...
                    val lesson = AppObjectController.appDatabase.lessonDao().getLesson(response.id)
                    if (lesson == null) {
                        AppObjectController.appDatabase.lessonDao().insertSingleItem(response)
    ...
    fun getRating() {
        ...
        if (PrefManager.getCallCount() >= 3) {
            ...
            {
                try {
                    val response = lessonNetworkService.getUserRating()
                    if (response.isSuccessful && response.body() != null) {
    ...
    private fun blockStatusFromApi(isForceHit: Boolean = false) {
        ...
        {
            try {
                val response = lessonNetworkService.getUserBlockStatus()
