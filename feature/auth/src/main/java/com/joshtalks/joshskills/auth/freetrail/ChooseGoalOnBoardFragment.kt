...
import com.joshtalks.joshskills.auth.freetrail.adapters.ChooseGoalAdapter
import com.joshtalks.joshskills.common.ui.special_practice.utils.ErrorView

class ChooseGoalOnBoardFragment : com.joshtalks.joshskills.common.base.BaseFragment() {
    ...
    fun onGoalSelected(goalSelectionResponse: GoalSelectionResponse) {
        if (goalSelectionResponse.testId == com.joshtalks.joshskills.auth.freetrail.ENGLISH_FOR_GOVERNMENT_EXAM_TEST_ID) {
        ...
        try {
            if (viewModel.abTestRepository.isVariantActive((VariantKeys.ENGLISH_FOR_GOVT_EXAM_ENABLED))) {
                (requireActivity() as com.joshtalks.joshskills.auth.freetrail.SignUpActivity).onGoalSelected(
                    goalSelectionResponse.testId ?: com.joshtalks.joshskills.auth.freetrail.HINDI_TO_ENGLISH_TEST_ID
                )
            }
        } catch (e: Exception) {
