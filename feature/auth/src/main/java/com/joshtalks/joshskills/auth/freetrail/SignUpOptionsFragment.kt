...
import com.joshtalks.joshskills.auth.databinding.FragmentSignUpOptionsBinding
...
class SignUpOptionsFragment : BaseSignUpFragment() {
    ...
    fun loginViaTrueCaller() {
        ...
        com.joshtalks.joshskills.common.messaging.RxBus2.publish(LoginViaEventBus(LoginViaStatus.TRUECALLER))
    }

    fun loginViaGoogle() {
        viewModel.loginAnalyticsEvent(LoginViaStatus.GMAIL.name)
        com.joshtalks.joshskills.common.messaging.RxBus2.publish(LoginViaEventBus(LoginViaStatus.GMAIL))
    }
    ...
    private fun callVerificationService() {
        (requireActivity() as com.joshtalks.joshskills.auth.freetrail.SignUpActivity).createVerification(
            prefix,
            binding.mobileEt.text!!.toString(),
