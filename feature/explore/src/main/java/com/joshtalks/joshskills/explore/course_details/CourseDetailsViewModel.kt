...
import com.joshtalks.joshskills.explore.ExploreNetwork
...
class CourseDetailsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun fetchCourseDetails(testId: String) {
        jobs += viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                val response = ExploreNetwork.exploreNetworkService.getCourseDetails(requestParams)
    ...
    fun fetchDemoCourseDetails() {
        jobs += viewModelScope.launch(Dispatchers.IO) {
            try {
                val response = ExploreNetwork.exploreNetworkService.getDemoCourseDetails()
                if (response.isSuccessful) {
                    response.body()?.let {
