...
public class VoiceRippleView extends View implements TimerCircleRippleRenderer.TimerRendererListener {
  ...
  private final Runnable updateRipple = new Runnable() {
    @Override
    public void run() {
      if (isRecording) {
  ...
  private void init(Context context, AttributeSet attrs) {
    MIN_RADIUS = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 32, getResources().getDisplayMetrics());
