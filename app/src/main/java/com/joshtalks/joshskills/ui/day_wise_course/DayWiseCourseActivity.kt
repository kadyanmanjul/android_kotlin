...
override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
    if (requestCode == LESSON_REQUEST_CODE && resultCode == AppCompatActivity.RESULT_OK && data?.hasExtra(
            IS_BATCH_CHANGED
        ) == true
    ) {
        setResult(AppCompatActivity.RESULT_OK, Intent().apply {
            putExtra(IS_BATCH_CHANGED, false)
            putExtra(LAST_LESSON_INTERVAL, lessonInterval)
            putExtra(DayWiseCourseActivity.LAST_LESSON_STATUS, lessonCompleted)
            putExtra(LESSON__CHAT_ID, chatId)
        })
        finish()
    }
...
override fun onBackPressed() {
    ...
    resultIntent.putExtra(IS_BATCH_CHANGED, isBatchChanged)
    resultIntent.putExtra(LAST_LESSON_INTERVAL, lessonInterval)
    resultIntent.putExtra(DayWiseCourseActivity.LAST_LESSON_STATUS, lessonCompleted)
    resultIntent.putExtra(LESSON__CHAT_ID, chatId)
