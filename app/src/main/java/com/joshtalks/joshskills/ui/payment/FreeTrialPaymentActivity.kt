...
class FreeTrialPaymentActivity : CoreJoshActivity(),
        ...
        override fun onReceive(context: Context, intent: Intent) {
            ...
            if (downloadID == id) {
                ...
                PdfViewerActivity.startPdfActivity(
                    ...
                    pdfPath = fileDir,
                    conversationId = this@FreeTrialPaymentActivity.intent.getStringExtra(
                        CONVERSATION_ID
                    )
                )
        ...
        if (intent.hasExtra(PaymentSummaryActivity.TEST_ID_PAYMENT)) {
            testId = if (PrefManager.getStringValue(FREE_TRIAL_TEST_ID).isEmpty().not()) {
                AppObjectController.getFirebaseRemoteConfig()
                    .getString(
                        FirebaseRemoteConfigKey.FREE_TRIAL_PAYMENT_TEST_ID + "_" + PrefManager.getStringValue(
                            FREE_TRIAL_TEST_ID
                        )
                    )
            } else {
    ...
    private fun performActionOnEnglishTapped() {
        ...
        if (isNewFlowActive) {
            englishCard.card.background =
                ContextCompat.getDrawable(this, R.drawable.white_rectangle_with_blue_bound_stroke)
        ...
        } else if (englishCard.iv_minimise.visibility == View.VISIBLE) performActionOntapped(
            englishCard
        )
    ...
    private fun performActionOnSubscriptionTapped() {
        ...
        } else if (subscriptionCard.iv_minimise.visibility == View.VISIBLE) performActionOntapped(
            subscriptionCard
        )
    }
    ...
    private fun setObservers() {
            ...
            try {
                ...
                if (isNewFlowActive) {
                    ...
                    it.combinedMessage?.get(1)?.let { list ->
                        subscriptionCard.course_info.visibility = View.VISIBLE
                        for (i in list.indices) {
                            val subscriptionTextView: AppCompatTextView =
                                createTextViewsDynamically()
                            subscriptionTextView.text = list[i]
                            subscriptionCard.course_info.addView(subscriptionTextView)
                        }
                    }
                    subscriptionCard.card.background = ContextCompat.getDrawable(
                        this,
        ...
        viewModel.abtestNewLayoutLiveData.observe(this) { abTestCampaignData ->
            ...
            if (!isNewFlowActive && isSyllabusActive) binding.syllabusLayout.visibility =
                View.VISIBLE
        }
        if(!isNetworkAvailable()){
    ...
    override fun onPaymentSuccess(razorpayPaymentId: String) {
        ...
        if (testId == freeTrialTestId) {
            ...
            if (isEnglishCardTapped && PrefManager.getBoolValue(IS_ENGLISH_SYLLABUS_PDF_OPENED)) {
                ...
                viewModel.postGoal(
                    GoalKeys.ESD_COURSE_BOUGHT.name,
