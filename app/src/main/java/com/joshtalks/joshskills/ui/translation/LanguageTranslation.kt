...
class LanguageTranslation(context: Context) : RelativePopupWindow(context) {

    init {
        @SuppressLint("InflateParams")
        contentView =
            LayoutInflater.from(context).inflate(R.layout.language_translation_popup, null)
    ...
    private fun circularReveal(anchor: View) {
        (contentView as CircularRevealCardView).run {
            post {
                ...
                measure(
                    makeDropDownMeasureSpec(this@LanguageTranslation.width, windowRect.width()),
                    makeDropDownMeasureSpec(this@LanguageTranslation.height, windowRect.height())
                )
