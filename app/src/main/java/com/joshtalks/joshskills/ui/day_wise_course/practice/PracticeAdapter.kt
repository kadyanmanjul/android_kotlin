...
class PracticeAdapter(
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder.itemViewType) {
            VOCAB_TYPE -> {
                ...
                (holder as QuizViewHolder).bind(itemList[position], position)
            }
        }
    }
    ...
    inner class QuizViewHolder(
        ...
        fun bind(chatModel: ChatModel, position: Int) {
            var assessmentQuestions: java.util.ArrayList<AssessmentQuestionWithRelations> =
                java.util.ArrayList()
                        ...
                        if (false) {
                            ...
                            if (response.isSuccessful) {
                                response.body()?.let {
                                    ...
                                    assessmentRelations =
                                        getAssessmentFromDB(chatModel.question?.assessmentId ?: 0)
                                    assessmentRelations?.questionList?.sortedBy { it.question.sortOrder }
                                        ?.forEach { item -> assessmentQuestions.add(item) }
