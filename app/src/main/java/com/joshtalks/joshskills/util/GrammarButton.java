...
public class GrammarButton extends androidx.appcompat.widget.AppCompatButton implements View.OnTouchListener {
    ...
    public GrammarButton(Context context) {
        super(context);
        init();
        this.setOnTouchListener(this);
    }

    public GrammarButton(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    ...
    public GrammarButton(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init() {
        ...
        mButtonColor = resources.getColor(R.color.grammar_button_default_color);
        mShadowColor = resources.getColor(R.color.grammar_button_default_shadow_color);
        mShadowHeight = resources.getDimensionPixelSize(R.dimen.grammar_button_default_shadow_height);
        mCornerRadius = resources.getDimensionPixelSize(R.dimen.grammar_button_default_conner_radius);
    }

    private void parseAttrs(Context context, AttributeSet attrs) {
        //Load from custom attributes
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.GrammarButton);
        if (typedArray == null) return;
        for (int i = 0; i < typedArray.getIndexCount(); i++) {
        int attr = typedArray.getIndex(i);
        if (attr == R.styleable.GrammarButton_shadowEnabled) {
        ...
        } else if (attr == R.styleable.GrammarButton_cornerRadius) {
            mCornerRadius = typedArray.getDimensionPixelSize(attr, R.dimen.grammar_button_default_conner_radius);
        }
    }
        typedArray.recycle();
