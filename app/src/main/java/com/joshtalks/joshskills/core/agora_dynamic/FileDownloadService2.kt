...
import com.joshtalks.joshskills.core.agora_dynamic.UnzipUtils.unzip
...
class FileDownloadService2 : Service() {

    private var mNotificationManager: NotificationManager? = null
    var libPath: String = AGORA_LIB_PATH
    ...
    private var downloadListener = object : FetchListener {
        ...
        override fun onCompleted(download: Download) {
            ...
            val file = File(libPath + File.separator + Build.SUPPORTED_ABIS[0] + File.separator + "agoraLib.zip")
            if(unzip(file,libPath)){
                AppObjectController.initAgoraPath()
                val broadcastIntent = Intent().apply {
                    action = CALLING_SERVICE_ACTION
                    putExtra(SERVICE_BROADCAST_KEY, START_SERVICE)
                }
                if(PrefManager.getVoipServiceStatus())
                    LocalBroadcastManager.getInstance(this@FileDownloadService2).sendBroadcast(broadcastIntent)
            }
            //decompress(file.absolutePath,libPath)
        }
    ...
    private fun showDownloadNotification() {
        ...
        startForeground(FileDownloadService2.NOTIFICATION_ID, lNotificationBuilder.build())
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            val directory = File(libPath + File.separator + Build.SUPPORTED_ABIS[0])
            ...
            val isAlreadyDownloaded = mDeviceFileListApp.size > 1
            if (isAlreadyDownloaded) {
                AppObjectController.initAgoraPath()
                mDeviceFileListApp.filter { it?.contains(".zip") == true }?.let {
                    it.forEach {
                        //Toast.makeText(this@FileDownloadService2, " device file and exit  $it", Toast.LENGTH_LONG).show()
                        AppDirectory.deleteFile(it)
                    }
                }
                stopForeground(true)
                stopSelf()
            } else {
                //Toast.makeText(this@FileDownloadService2, " device file  $mDeviceFileListApp", Toast.LENGTH_LONG).show()
                try {
                    val resp =
                        AppObjectController.commonNetworkService.getAgoraLibPath(Build.SUPPORTED_ABIS[0].toString())
    ...
    suspend private fun downloadZip(zipUrl: String) {
        val name = "agoraLib.zip"
        val directory = File(libPath + File.separator + Build.SUPPORTED_ABIS[0])
        if (!directory.exists()) {
            if (directory.mkdirs()) //directory is created;
                Log.i(TAG, "directory dir created")
            else Log.w(TAG, "Unable to create directory dir!")
        }
        val path = File(libPath + File.separator + Build.SUPPORTED_ABIS[0] + File.separator + name)
        if (!path.exists()) {
        ...
        val request = Request(zipUrl, path.toString())
        request.priority = Priority.HIGH
        request.networkType = NetworkType.ALL
        request.tag = zipUrl

        AppObjectController.getFetchObject().addListener(downloadListener)
        DownloadUtils.objectFetchListener[zipUrl] = downloadListener
    ...
    fun decompress(zipFile: String, location: String) {
        try {
            ...
            while (zin.getNextEntry().also { ze = it } != null) {
                if (ze.isDirectory()) {
                    ...
                    val fout = FileOutputStream(File(location, ze.getName()))
            ...
            Log.d(TAG, "decompress() called with: catch error = $e ")
        } finally {
            Log.d(TAG, "finally decompressed  = ${location + File.separator + getFileName(zipFile)} ")
        }
    }

    private fun isValidDir(saveDir: String): Boolean {
        return isValidDir(saveDir, "")
    }

    private fun isValidDir(location: String, dir: String): Boolean {
        if (!isValidStr(location)) return false
        val f = File(location, dir)
        if (!f.exists()) f.mkdirs()
        return true
    }

    private fun getCompressFileName(downloadUrl: String): String? {
        val lastIndex = downloadUrl.lastIndexOf(File.separator)
        return downloadUrl.substring(lastIndex + 1, downloadUrl.length)
    }

    private fun getFileName(downloadUrl: String): String? {
        val lastIndex = downloadUrl.lastIndexOf(File.separator)
        val zipName = downloadUrl.substring(lastIndex + 1, downloadUrl.length)
        val nameIndex = zipName.indexOf('.'.code.toChar())
    ...
    private fun getDeviceFiles(): ArrayList<String?> {
        ...
        if (directory.length() != 0L) // check no of files
        {
            for (file in directory.listFiles()!!) {
                Log.d(TAG, "downloaded files called ${file.path}")
                if (file.isFile) deviceFileList.add(file.name)
            }
        }
        Log.d(TAG, "getDeviceFiles() called $directory $deviceFileList")
    ...
    companion object {
        ...
        fun uploadAllPendingTasks(context: Context) {
            try {
                val intent = Intent(context, FileDownloadService2::class.java)
