...
open class RadialProgressBar : View {
    ...
    private var mEmptyProgressColorInnerMostView = Color.parseColor("#7B2ECB")
    private var mInnerMostColor = ArrayList<Int>()
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        when {
            ...
            !hasOneProgressView && !hasTwoProgressView -> {
                drawInnerMostProgressView(canvas)
    ...
    private fun parseAttributes(a: TypedArray) {
        ...
        mInnerMostProgress =
            a.getInteger(R.styleable.RadialProgressBar_innerMostProgress, mInnerMostProgress)
        mProgressColorInnerMostView = a.getColor(
            R.styleable.RadialProgressBar_innerMostProgressColor,
            mProgressColorInnerMostView
        )
        mCenterProgress =
        ...
        mStartAngleInnerMostView = a.getInteger(
            R.styleable.RadialProgressBar_innerMostProgressStartAngle,
            mStartAngleInnerView
        )
        ...
        mMaxProgressInnerView =
            a.getInteger(R.styleable.RadialProgressBar_innerMaxProgress, mMaxProgressInnerMostView)
        mMaxProgressInnerMostView = a.getInteger(
            R.styleable.RadialProgressBar_innerMostMaxProgress,
            mMaxProgressInnerMostView
        )
        ...
        mEmptyProgressColorInnerMostView =
            a.getColor(
                R.styleable.RadialProgressBar_innerMostEmptyProgressColor,
                mEmptyProgressColorInnerMostView
            )
        mCircleThickness =
        ...
        setEmptyProgressColorInnerMostView(mEmptyProgressColorInnerMostView)
        ...
        setMaxProgressInnerMostView(mMaxProgressInnerMostView)
        ...
        setInnerMostProgress(mInnerMostProgress)
        mInnerColor.add(mProgressColorInnerView)
        mInnerMostColor.add(mProgressColorInnerMostView)
        setInnerProgressColor(mInnerColor)
        setInnerMostProgressColor(mInnerMostColor)
        ...
        setStartAngleInnerMostView(mStartAngleInnerMostView)
     ...
     */
    private fun drawInnerMostProgressView(canvas: Canvas?) {
        ...
        val addVal = (stroke * 3) + 3 * mCirclePadding
        val subVal = ((stroke * 3) + paddingView + 3 * mCirclePadding)
        val oval =
            RectF(paddingView + addVal, paddingView + addVal, diameter - subVal, diameter - subVal)
        mPaintInnerMostView.strokeWidth = stroke
        mPaintInnerMostView.isAntiAlias = true
        mPaintInnerMostView.strokeCap = if (mRoundedCorners) Paint.Cap.ROUND else Paint.Cap.BUTT
        mPaintInnerMostView.style = Paint.Style.STROKE
        if (mElevation) {
            setLayerType(View.LAYER_TYPE_SOFTWARE, mPaintInnerMostView)
            mPaintInnerMostView.setShadowLayer(
        ...
        when {
            mInnerMostColor.size == 1 -> mPaintInnerMostView.color = mInnerMostColor[0]
            mInnerMostColor.size == 2 -> mPaintInnerMostView.shader = LinearGradient(
                0f, 0f, 0f, height.toFloat(),
                mInnerMostColor[0],
                mInnerMostColor[1],
                Shader.TileMode.MIRROR
            )
            mInnerMostColor.size > 2 -> {
                mPaintInnerMostView.shader = LinearGradient(
                    0f, 0f, 0f,
                    height.toFloat(),
                    mInnerMostColor[0],
                    mInnerMostColor[1],
                ...
                Log.e("RadialProgressBar", "InnerMost Progress Color Can't Be more then Two colors")
            }
            mInnerMostColor.size == 0 -> mPaintInnerMostView.color = mProgressColorInnerMostView
        }
        canvas?.drawArc(
            oval,
            mStartAngleInnerMostView.toFloat(),
     ...
     * @return Sweep Angle for Inner ProgressView
     * */
    private fun calcSweepAngleFromInnerMostProgress(progress: Int): Float {
        return when {
            !(hasOneProgressView && hasTwoProgressView) -> (mMaxSweepAngle / mMaxProgressInnerMostView * progress)
            else -> 0F
        }
    }
    ...
    /**
    Set the  Color for Empty InnerMost Progress View
     */
    fun setEmptyProgressColorInnerMostView(value: Int) {
        if (!(hasOneProgressView && hasTwoProgressView)) {
            mEmptyProgressColorInnerMostView = value
            invalidate()
        }
    }
    ...
    /**
    Set the Start Angle for InnerMost Progress View
     */
    fun setStartAngleInnerMostView(angle: Int) {
        if (!(hasOneProgressView && hasTwoProgressView)) {
            mStartAngleInnerMostView = angle
            invalidate()
        }
    }
    ...
    /**
    Set the  Maximum Progress of InnerMost Progress View
     */
    fun setMaxProgressInnerMostView(max: Int) {
        if (!(hasOneProgressView && hasTwoProgressView)) {
            mMaxProgressInnerMostView = max
            invalidate()
        }
    }
    ...
    /**
    set the progress  for innerMost progressview
     */
    fun setInnerMostProgress(progress: Int) {
        if (!(hasOneProgressView && hasTwoProgressView)) {
            if (progress != 0) mInnerMostProgress = progress
            val animator =
                ValueAnimator.ofFloat(
                    mSweepAngleInnerMostView.toFloat(),
            ...
            animator.addUpdateListener { valueAnimator ->
                val value: Float = valueAnimator.animatedValue as Float
                mSweepAngleInnerMostView = (value.toInt())
                invalidate()
    ...
    /**
    set the progress color for inner progressview
     */
    ...
    /**
    set the progress color for innerMost progressview
     */
    fun setInnerMostProgressColor(color: ArrayList<Int>) {
        if (!(hasOneProgressView && hasTwoProgressView)) {
            mInnerMostColor = color
            invalidate()
        }
    }
