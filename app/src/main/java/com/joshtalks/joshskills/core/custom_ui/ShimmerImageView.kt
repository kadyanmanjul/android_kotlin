 ...
 */
data class MaskSpecs(
    val maskColor: Int = DEFAULT_MASK_COLOR,
...
class ShimmerImageView @JvmOverloads constructor(
    ...
    init {
        attrs?.let {
            ...
            val maskColor =
                a.getColor(R.styleable.ShimmerImageView_maskSpecs_maskColor, DEFAULT_MASK_COLOR)
            val intensity =
                a.getFloat(R.styleable.ShimmerImageView_maskSpecs_intensity, DEFAULT_INTENSITY)
            val dropOff =
                a.getFloat(R.styleable.ShimmerImageView_maskSpecs_dropOff, DEFAULT_DROPOFF)
            val maskMode =
                parseMaskMode(a.getInt(R.styleable.ShimmerImageView_maskSpecs_maskMode, -1))
            val animationDuration = a.getInt(
                R.styleable.ShimmerImageView_maskSpecs_animationDuration,
                DEFAULT_ANIMATION_DURATION
            )
            val startDelayed =
                a.getInt(R.styleable.ShimmerImageView_maskSpecs_startDelayed, DEFAULT_START_DELAYED)
    ...
    fun startAnimation() {
        ...
        runAfterLaidOut {
            ...
            val startWidth =
                width + (maskSpecs.startDelayed / maskSpecs.animationDuration.toFloat()).toInt()
    ...
    private fun getGradientPositions(): FloatArray {
        return floatArrayOf(
            Math.max(0f, maskSpecs.dropOff), 0.5f - maskSpecs.intensity / 2f, 0.5f + maskSpecs
